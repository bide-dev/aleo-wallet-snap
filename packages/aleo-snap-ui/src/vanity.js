...
export const findAddressContainingSubstring = async (substr, maxEpoch = 10) => {
  ...
  if (substr.length >= 9) {
    console.log(
      "Warning, vanities over 9 characters are expected to take longer than 3 hours and may timeout."
    );
    console.log("Strongly suggest reducing the input size to below 9 chars");
  }
  const ALPHABET =
    "0123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijlkmnopqrstuvwxyz";
  ...
  for (let i = 0; i < substr.length; i++) {
    if (
      ...
      console.log(
        "Your search term is not valid - please ensure search term only includes bech32 valid characters: " +
          ALPHABET
      );
